...
const EquipesMensal = (() => {
  ...
  function renderTabela(metaId, metaData) {
    ...
    for (let m = 1; m <= 12; m++) { html += linhaMes(m, mensal[m], cols, metaId); }
    html += '</tbody></table>';
    html += '<div class="eq-detalhe-container" data-detalhe-meta="' + metaId + '"></div>';
    return html;
  }

  function linhaMes(m, d, cols, metaId) {
    if (!d) {
      const v = cols.map(() => '<td>-</td>').join('');
      return '<tr class="eq-tr--vazio"><td>' + MESES[m] + '</td>' + v + '<td>-</td><td></td></tr>';
    }
    ...
    return '<tr class="' + cls + '"><td>' + MESES[m] + '</td>' + cells +
      ...
      '<td><button class="eq-btn-detalhe" data-meta="' + metaId + '" data-mes="' + m +
      '">Ver</button></td></tr>';
  }
