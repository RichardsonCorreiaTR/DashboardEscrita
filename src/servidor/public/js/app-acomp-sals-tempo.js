/**
 * app-acomp-sals-tempo.js - Tempo por SAL (detalhamento mensal)
 *
 * Filtros: analista, nivel, ano. Tabela por mes (acumulado) com botao "Ver"
 * que expande os SAIs do mes. Grafico de tendencia da media de analise/SAL.
 */
/* global Chart */
const AppAcompSalsTempo = (() => {
  ...
  async function consultar() {
    ...
    const nivel = document.getElementById('salt-nivel').value;
  ...
  function renderDetalhe(linhas) {
    const rows = linhas
      ...
      .map(l => `
        <tr>
          ...
          <td>${l.i_psai}</td>
          <td>${l.i_sai || '—'}</td>
          <td><span class="salt-badge" style="background:${(NIVEL_COR[l.nivel] || '#94a3b8')}22;color:${NIVEL_COR[l.nivel] || '#64748b'}">${l.nivel_nome}</span></td>
    ...
    return `
      <table class="salt-detalhe">
        <thead><tr>
          <th>Analista</th><th>PSAI</th><th>SAI</th><th>Nível</th>
  ...
  // Linha de tendencia (regressao linear simples) para indicar aumento/queda
