/**
 * acomp-helpers.js - Queries Acomp. NEs/SALs filtradas por um unico codigo-sgd
 */
const fs = require('fs');
const path = require('path');
const { ROOT } = require('./config-analista');

const AREA_SAL = "sp.nomeArea IN ('Escrita', 'Importacao', 'ONVIO ESCRITA')";
...
function qe() {
  return require(path.join(ROOT, 'src/core/query-executor'));
}

function queryTempoAtivas(ano, sgd) {
  return `
    ...
    WHERE ${AREA_SAL} AND sp.tipoSAI = 'SAL' AND sp.i_psai_situacoes NOT IN ${SITS_DESC}
...
function queryTempoDesc(ano, sgd) {
  return `
    SELECT p.i_responsaveis as i_usuarios, MONTH(sp.CadastroPSAI) as mes,
    ...
    JOIN bethadba.psai p ON sp.i_psai = p.i_psai
    LEFT JOIN bethadba.psai_responsaveis pr ON pr.i_psai = sp.i_psai AND pr.i_usuarios = p.i_responsaveis
    WHERE ${AREA_SAL} AND sp.tipoSAI = 'SAL' AND sp.i_psai_situacoes IN ${SITS_DESC}
      AND COALESCE(p.i_produto_grupo, 1) = 1 AND p.i_responsaveis = ${sgd}
      AND YEAR(sp.CadastroPSAI) = ${ano}
    GROUP BY p.i_responsaveis, MONTH(sp.CadastroPSAI)`;
}
...
async function tempoDescarte(slug, ano) {
  ...
  const [rA, rD, rAB, rDB] = await Promise.all([
    exec.executar(queryTempoAtivas(ano, sgd)),
    exec.executar(queryTempoDesc(ano, sgd)),
    exec.executar(queryTempoAtivas(anoBase, sgd)),
    exec.executar(queryTempoDesc(anoBase, sgd))
  ]);
...
async function linhasSal(slug, ano, nivel) {
  const colab = lerColab(slug);
  if (!colab) throw new Error('Colaborador nao encontrado');
  ...
  const sql = `
    ...
    WHERE ${AREA_SAL} AND sp.tipoSAI = 'SAL' AND COALESCE(p.i_produto_grupo, 1) = 1
...
module.exports = { lerColab, tempoDescarte, linhasSal, linhasNe, filtrosSelf, NIVEL_NOME };
