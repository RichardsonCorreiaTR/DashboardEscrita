...
const AppEstudos = (() => {
  ...
  async function carregarResumoExecutivo(nomeVersao, isv) {
    ...
    try {
      ...
      const outrosDiv = document.getElementById('resumo-executivo-outros');
      const r = data.resumo;
      ...
      renderizarOutros(outrosDiv, data.outros || []);
    } catch (err) {
      section.hidden = true;
  ...
  function renderizarOutros(el, outros) {
    el.innerHTML = '';
    if (!outros.length) return;
    const URL_PSAI = 'https://sgd.dominiosistemas.com.br/sgsa/faces/psai.html?psai=';
    const semDesc = outros.filter(n => !n.descricao);
    const rows = outros.map(n => {
      const sscBadge = n.sscs > 0
        ? `<span style="background:var(--cor-bg-alt);padding:0 4px;border-radius:3px;font-size:0.75rem">${n.sscs} SSC</span>` : '-';
      const corGrav = n.gravidade === 'Critica' ? 'var(--vermelho)' : n.gravidade === 'Grave' ? 'var(--amarelo)' : '';
      const gravBadge = corGrav ? `<span style="color:${corGrav};font-weight:600;font-size:0.75rem">[${n.gravidade}]</span> ` : '';
      const link = `<a href="${URL_PSAI}${n.i_psai}" target="_blank" class="link-sgd" style="font-size:0.75rem">${n.i_psai}</a>`;
      const descTexto = n.descricao
        ? `${gravBadge}${n.descricao}`
        : `${gravBadge}<em style="opacity:0.5">(sem descricao - PSAI sem SAI vinculada)</em>`;
      return `<tr>
        <td>${link}</td>
        <td style="font-size:0.78rem;max-width:480px">${descTexto}</td>
        <td style="text-align:center">${sscBadge}</td>
      </tr>`;
    }).join('');
    const aviso = semDesc.length > 0
      ? `<span style="color:var(--amarelo);font-size:0.78rem;margin-left:0.8rem">${semDesc.length} sem descricao (i_sai=0)</span>`
      : '';
    el.innerHTML = `
      <strong>Detalhamento - NEs sem Classificacao (Outros):</strong>
      <span style="opacity:0.6;font-size:0.8rem;margin-left:0.4rem">${outros.length} NE(s)</span>${aviso}
      <table class="tabela-detalhes" style="font-size:0.8rem;margin-top:0.4rem">
        <thead><tr><th>PSAI</th><th>Descricao</th><th>SSCs</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>`;
  }
