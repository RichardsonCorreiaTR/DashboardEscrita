...
const Detalhes = (() => {
  ...
  function render95d(r, body) {
    ...
    body.innerHTML = `
      ...
      <div class="info-grid">
        ${infoBox('Total > 95d', r.valor)}
        ${infoBox('Meta', r.meta)}
        ${faixas.map(f => infoBox(f.faixa, f.qtd)).join('')}
      </div>
      <div class="grafico-container"><canvas id="chart-faixas"></canvas></div>
      ${abasHTML([
        { id: 'ne95-antigas', titulo: 'Top 10 Mais Antigas', qtd: (d.top_10_mais_antigas||[]).length, html: tabela(colsNE, d.top_10_mais_antigas||[]) },
        { id: 'ne95-recem', titulo: 'Recem-incluidas (96-120d)', qtd: (d.recem_incluidas||[]).length, html: tabela(colsNE, d.recem_incluidas||[]) },
        { id: 'ne95-prox', titulo: 'Proximas a entrar (65-95d)', qtd: (d.proximas_entrar||[]).length, html: tabela(colsNE, d.proximas_entrar||[]) }
      ])}
    `;
    if (faixas.length > 0) Charts.barrasFaixas('chart-faixas', faixas);
    inicializarAbas(body);
  }
  ...
  function renderTempo(r, body) {
    ...
    body.innerHTML = `
      <div class="info-grid">
        ...
        ${infoBox('SAIs NE', d.total_sai_ne)}
  ...
  function renderEntradas(r, body) {
    ...
    body.innerHTML = `
      ...
      ${abasHTML([
        { id: 'ent-situacao', titulo: 'Por Situacao', qtd: descSit.length,
          html: tabela([
